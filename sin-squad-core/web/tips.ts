...
const TIPS: Record<TipKey, [string, string]> = {
  ...
  place: ["布阵", "从候选人物中选 3 名放到 1、2、3 号位，再点其中一名设为亮出——对手只看得到这一名。默认每个位置攻击对面同号位；操作阶段可以公开改一次指向。"],
  peek: ["窥视", "你的密探可以偷看对手一张暗牌，再决定要不要交换自己两名暗置人物的位置。对手不会知道。"],
  reveal2: ["再翻开一名", "双方同时暗选再翻开一名，然后一起翻开。强的可以留到最后，也可以先亮出来吓唬对手。"],
  bet: ["下注", "和德州扑克一样：过牌、跟注、加注或弃牌。弃牌会输掉已经投入的筹码。第 1 轮下注后翻开胜利规则和公共效果。"],
  operate: ["操作", "操作费等于本轮下注匹配额。你可以付费拿一件装备，或付同样费用公开改三个位置的优先目标；两者只能选一个。"],
  ...
  battle: ["战斗", "双方轮流出手。每个位置默认攻击对位，也会按照你公开指定的优先目标攻击；被打的人会把自己的攻打回来。先达成胜利规则的一方赢下奖池。"],
};
